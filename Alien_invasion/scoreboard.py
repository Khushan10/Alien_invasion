...
class Scoreboard():
	...
	def prep_level(self):
		self.level_image = self.font.render("Level: " + str(self.stats.level), True, self.text_color, self.ai_setting.bg_color)
	...
	def prep_ships(self):
		...
		for ship_number in range(self.stats.ships_left):
			shp = Ship(self.ai_setting, self.screen)
			shp.rect.x = 10 + ship_number*shp.rect.width
			shp.rect.y = 10
			self.ships.add(shp)
